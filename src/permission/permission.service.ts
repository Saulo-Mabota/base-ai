...
export class PermissionService {
  ...
  async createFullPermission(createPermissionDto: CreatePermissionDto) {
    try {
      const lastPermission = await this.permission.findOne({
        order: { createdAt: 'desc' },
      });

      const lastPosition = lastPermission ? lastPermission.position : 0;
      const permission = await this.permission
        ...
        .then(async (permission) => {
          permission.id = permission._id;
          const labelPermission = await this.labelPermission.create({
            description: createPermissionDto.description,
            lang: 'pt',
            title: createPermissionDto.name,
            link: createPermissionDto.name,
            isHeadr: false,
            icon: 'heroicons:home-solid',
            position: lastPosition + 1,
            permissionId: permission.id,
          ...
          console.log('labelPermission', labelPermission);
